...
define(
    function (require) {
        ...
        TransitObject.prototype.getProgress = function () {
            var progress = Math.max(0, Math.min(1, this._position / this.lastArguments.duration));
            return Math.round(progress * 1000) / 1000;
        };

        var easingFunc; // easing的处理函数
        ...
        TransitObject.prototype.pause = function () {
            ...
            for (var name in line.properties) {
                if (isTransformProperty(name)) {
                    ...
                    if (lowName === 'x' || lowName === 'y' || lowName === 'z') {
                        var transform = getProperty(targetStyle, 'transform')[1];
                        var transformValues = transform.substring(transform.indexOf('(') + 1, transform.length - 1)
                            .split(/\s*,\s*/);
                        var offset = transform.indexOf('matrix3d') === 0 ? 12 : 4;
                        var currentOffset = offset + (lowName === 'z' ? 2 : (lowName === 'y' ? 1 : 0));
                        ...
                        if (!easingFunc) {
                            ...
                            var valueProgress = easingFunc(progress);
                        }
                        var beginValue = parseFloat(line.propertyData[name].begin);
                        var endValue = parseFloat(line.propertyData[name].end);
                        var current = ((endValue - beginValue) * valueProgress) + beginValue;
                        style[name] = current;
                    }
                }
                else {
        ...
        function getNewBezier(oldBezier, time) {
            ...
            var p0y = 0;
            var p1x = u;
            var p1y = (oldBezierFunc(uu) - startY) * sign / yInterval;
            var p2x = v;
            var p2y = (oldBezierFunc(vv) - startY) * sign / yInterval;
            var p3x = 1;
            var p3y = 1;
            var compU = 1 - u;
            var compV = 1 - v;
            ...
            var v2 = v * v;
            var v3 = v * v * v;
            var a = 3 * compU * compU * u;
            var b = 3 * compU * u2;
            var c = 3 * compV * compV * v;
            var d = 3 * compV * v2;
            ...
            var compU3 = compU * compU * compU;
            var compV3 = compV * compV * compV;

            var q1x = p1x - (compU3 * p0x + u3 * p3x);
            var q1y = p1y - (compU3 * p0y + u3 * p3y);
            var q2x = p2x - (compV3 * p0x + v3 * p3x);
            var q2y = p2y - (compV3 * p0y + v3 * p3y);
        ...
        function bezier(x1, y1, x2, y2) {
            ...
            var xDeriv = function (time) {
                return C[0] + time * (2 * B[0] + 3 * A[0] * time);
            };

            var xForT = function (time) {
                var x = time;
                var i = 0;
                var z;
                while (++i < 14) {
        ...
        var propertyNames = {}; // 存放处理的propertyNames
        ...
        TransitObject.prototype.transit = function (properties, duration, easing, callback, type) {
            ...
            if (typeof duration === 'undefined') {
                duration = option.speeds.defaultSpeed;
            }
            if (typeof duration === 'object') {
                delay = util.toMS(duration.delay) || '0ms';
                duration = util.toMS(duration.duration) || '0ms';
            }
            if (typeof easing === 'undefined' || !easing) {
                easing = map.cssEase.defaultSpeed;
            }
